#ifndef LABA5_CIRCULARBUFFER_H
#define LABA5_CIRCULARBUFFER_H
...
{
public:
     classIterator : public iterator<random_access_iterator_tag, T>
    {
    private:
 T *iter;
    ...
    public:
        ...
        explicit Iterator(T *other) : iter(other) {} 

        Iterator(const Iterator &other) : iter(other.iter) {}

        distanse operator-(const Iterator &other) const { return iter - other.iter; } 

        Iterator operator+(distanse other) const { return Iterator(iter + other); } 

        Iterator operator-(distanse other) const { return Iterator(iter - other); } 

        friend Iterator operator+(distanse a, const Iterator &b) { return Iterator(a + b.iterator); } 
